...
\usepackage{color}
\definecolor{backgcode}{rgb}{.85,.85,.85}
\usepackage{fullpage}
\usepackage{listings}
\lstset{ %
language=R,
...
backgroundcolor=\color{backgcode},
postbreak=\space,
breakindent=5pt,
breaklines,
frame=single
...
\lstinputlisting[caption=Esempio di un boxplot,label=lst:boxplot]{snippets/boxplot.R}
...
\lstinputlisting[caption=Esempio anscombe,label=lst:anscombe]{snippets/anscombe.R}

\marginpar{Hint: per ripristinare il parametro mfrow come al default (1 plot per window), > par(mfrow=c(1,1));}5~
...
\lstinputlisting[caption=Esempio di dispersione,label=lst:dispersione]{snippets/dispersione.R}
...
\lstinputlisting[caption=Esempio]{snippets/ex1.R}
